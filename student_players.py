...
class Xerecuda(Player):
    def play(self, board_extremes, play_hist):
        ...
        if len(board_extremes) == 0:  # Condição para saber se o jogador é o primeiro a jogar.
            # Implementar uma estratégia para quando se começa primeiro.
            for tile in playable_tiles:
                if tile[0] == tile[1]:
                    return 1, tile
                else:
                    # Caso não encontre uma peça dupla, prossegue com a lógica abaixo.
...
def create_pair():
    return (Xerecuda("123", 'Agnes'), Xerecuda('321', 'Marlon'))  # Defina aqui a dupla de jogadores. Deve ser uma tupla com dois jogadores.